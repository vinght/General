...
# データの種類
number = 100 		#数値(integer)
strings = "paiza"	#文字列(string)
print(number)
print(strings)
'''    
上記のプログラムを実行すると数値と文字列がそれぞれprintされる
なんでこんな書き方の違いがあるかって？
それはプログラミングではinteger_type(整数or数列)とstring_type(文字列)
といったデータの種類を厳密に区別してるからだよ
このようなデータの種類を「データ型(Data_type)」と呼ぶ
'''    
...
'''
じゃあ例えば数値から文字へのデータ型変換はstr()で出来るのはご周知の通りだが
